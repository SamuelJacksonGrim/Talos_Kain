...
> **Status:** design specification, mostly unbuilt. A milestone-zero vertical slice implements a thin path through §3 (episodic and semantic stores), §7 (skill extraction and lifecycle) and §14 (a verifying audit chain), and it learns on a mock environment. Everything else — sleep, wake, the identity crucible, telos, the federated cortex — is a typed stub. **Treat every mechanism described here as a hypothesis unless `talos/` implements it.** The structural invariants below are checked against this document's own diagram by `tools/invariant_lint.py`; nothing yet checks them against the code.
>
> **Compass:** `docs/north_star.md` — *learn by losing, and be able to show it.* The one-gate law in this document is what makes running that unattended safe; it is not what the running is for.
>
